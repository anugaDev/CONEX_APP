...
public class TutorsReportGenerator : PdfReportBase
{
    public string GenerateReport(List<UserDto> tutors)
    {
        string filePath = GetFilePath("Listado_Enseñantes");
        ...
        Document document = Document.Create(container =>
        {
            container.Page(page =>
            {
                ...
                page.Header().Element(c => ComposeHeader(c, "Listado de Enseñantes"));
                page.Content().Element(c => ComposeUserTable(c, tutors));
                ComposeFooter(page);
            });
        });
